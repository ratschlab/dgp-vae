...
for n in ${!seeds[@]}; do
  let n+=1
  for epochs in 14 19; do
    bsub -o models/gpvae/epoch_sweep0/epoch_"$epochs"/log_"$epochs"_"$n" -g /gpvae_disent -W 7:00 \
    ...
    --seed "${seeds[$n-1]}" --banded_covar --latent_dim 64 --encoder_sizes=32,256,256 \
