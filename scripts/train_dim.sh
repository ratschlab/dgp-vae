...
for dim in 64; do
  mkdir -p models/dim/dsprites_dim_"$dim"_2
done
...
for n in {1..10}; do
  ...
  bsub -o models/dim/dsprites_dim_"$dim"_2/log_"$dim"_$n -g /gpvae_disent \
  ...
  --exp_name dsprites_"$dim"_n$n --basedir models/dim/dsprites_dim_"$dim"_2 \
