...
for n in 1; do
  ...
  bsub -o models/dsprites_dim_"$dim"_debug2/lfs_log_"$dim"_$n -g /gpvae_disent -R "rusage[mem=16000,ngpus_excl_p=1]" -R "select[gpu_model0==GeForceGTX1080Ti]"\
  python train.py --model_type gp-vae --data_type dsprites --exp_name dsprites_"$dim"_debug_n$n \
  --basedir models/dsprites_dim_"$dim"_debug2 --seed $SEED --banded_covar --latent_dim $dim \
