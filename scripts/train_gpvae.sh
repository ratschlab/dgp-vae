...
for dim in 8 16 32 64 128; do
  mkdir -p models/dsprites_dim_"$dim"_tensor2
done
...
for n in {1..10}; do
  ...
  bsub -o models/dsprites_dim_"$dim"_tensor2/lfs_log_"$dim"_$n -g /gpvae_disent -R "rusage[mem=16000,ngpus_excl_p=1]" -R "select[gpu_model0==GeForceGTX1080Ti]"\
  python train.py --model_type gp-vae --data_type dsprites --exp_name dsprites_"$dim"_n$n \
  --basedir models/dsprites_dim_"$dim"_tensor2 --seed $SEED --banded_covar --latent_dim $dim \
