...
for n in {1..10}; do
  ...
  bsub -o models/dsprites_dim_"$dim"_gp_full1/lfs_log_"$dim"_$n -g /gpvae_disent -R "rusage[mem=16000,ngpus_excl_p=1]" -R "select[gpu_model0==GeForceGTX1080Ti]"\
  python train.py --model_type gp-vae --data_type dsprites --exp_name dsprites_"$dim"_n$n \
  --basedir models/dsprites_dim_"$dim"_gp_full1 --seed $SEED --banded_covar --latent_dim $dim \
