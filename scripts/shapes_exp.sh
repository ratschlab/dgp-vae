...
for n in {1..10};do
  ...
  for num in 1 2;do
    for len in 5 10;do
      bsub -o models/shapes_part"$num"/base/len_"$len"/same/log_%J -g /gpvae_disent \
      ...
      --data_dir /cluster/work/grlab/projects/projects2020_disentangled_gpvae/data/shapes3d/shapes_part"$num".npz \
      --exp_name n_"$n" --basedir models/shapes_part"$num"/base/len_"$len"/same \
      ...
      --num_epochs 1 --kernel cauchy --c_path /cluster/work/grlab/projects/projects2020_disentangled_gpvae/data/shapes3d/factors_shapes_part"$num".npz \
      --save_score

      bsub -o models/shapes_part"$num"/base/len_"$len"/scaled/log_%J -g /gpvae_disent \
      -R "rusage[mem=220000,ngpus_excl_p=1]" -R "select[gpu_model0==GeForceGTX1080Ti]" \
      python run_experiment.py --model_type gp-vae --data_type shapes3d --time_len "$len" --testing --batch_size 32 \
      --data_dir /cluster/work/grlab/projects/projects2020_disentangled_gpvae/data/shapes3d/shapes_part"$num".npz \
      --exp_name n_"$n" --basedir models/shapes_part"$num"/base/len_"$len"/scaled \
