...
def aggregate(N, latent_dims, base_dir='dsprites_dim_'):
    ...
    for dim, m in latent_dims:
        dim_dir = base_dir+"{}".format(dim)
        n = 0
        for _, dirs, _ in os.walk(os.path.join("models", dim_dir)):
            dci_file = glob(os.path.join(dirs,'dci*')) # This should find the file that begin with dci so full name doesnt have to be specified MUST STILL BE TESTED
            print(dci_file)
